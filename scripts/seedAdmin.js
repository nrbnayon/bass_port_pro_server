...
const ADMIN_ATOMS = [
  'view_dashboard',
  'manage_lakes',    'view_lakes',
  'manage_catches',  'view_catches',
...
const seedAdmin = async () => {
  ...
  try {
    ...
    if (!adminUser) {
      ...
      if (adminUser.status !== 'active'){ adminUser.status = 'active'; changed = true; }
      if (changed) { await adminUser.save(); console.log('🔧 Admin role/status fixed'); }
